      ...
      create or replace  table "THUNIVU".COVID."COVID_VIRUS"  as
      (select * from(
            
with __dbt__cte__COVID_VIRUS_AB1 as (

-- SQL model to parse JSON blob stored in a single column and extract into separated field columns as described by the JSON Schema
-- depends_on: "THUNIVU".COVID._AIRBYTE_RAW_COVID_VIRUS
select
...
from "THUNIVU".COVID._AIRBYTE_RAW_COVID_VIRUS as table_alias
-- COVID_VIRUS
where 1 = 1

),  __dbt__cte__COVID_VIRUS_AB2 as (

-- SQL model to cast each column to its adequate SQL type converted from the JSON schema type
-- depends_on: __dbt__cte__COVID_VIRUS_AB1
select
    cast(KEY as 
    varchar
) as KEY,
    ...
    _AIRBYTE_EMITTED_AT,
    convert_timezone('UTC', current_timestamp()) as _AIRBYTE_NORMALIZED_AT
from __dbt__cte__COVID_VIRUS_AB1
-- COVID_VIRUS
where 1 = 1

),  __dbt__cte__COVID_VIRUS_AB3 as (

-- SQL model to build a hash column based on the values of this record
-- depends_on: __dbt__cte__COVID_VIRUS_AB2
select
    ...
    varchar
)) as _AIRBYTE_COVID_VIRUS_HASHID,
    tmp.*
from __dbt__cte__COVID_VIRUS_AB2 tmp
-- COVID_VIRUS
where 1 = 1

)-- Final base SQL model
-- depends_on: __dbt__cte__COVID_VIRUS_AB3
select
    KEY,
    DATE,
    NEW_TESTED,
    ...
    _AIRBYTE_EMITTED_AT,
    convert_timezone('UTC', current_timestamp()) as _AIRBYTE_NORMALIZED_AT,
    _AIRBYTE_COVID_VIRUS_HASHID
from __dbt__cte__COVID_VIRUS_AB3
-- COVID_VIRUS from "THUNIVU".COVID._AIRBYTE_RAW_COVID_VIRUS
where 1 = 1

            ) order by (_AIRBYTE_EMITTED_AT)
      );
    alter table "THUNIVU".COVID."COVID_VIRUS" cluster by (_AIRBYTE_EMITTED_AT);